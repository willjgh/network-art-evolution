...
# current grid
current_grid = np.zeros((canvas_height, canvas_width, 3)) 

# next grid
next_grid = np.zeros((canvas_height, canvas_width, 3))
...
while running:
    ...
    if len(grid_list) == 0:
        ...
        # compute new next grid
        u1 = rng.integers(1, 15, 5)
        u2 = rng.integers(1, 15, 5)
        u3 = rng.integers(1, 15, 5)
        for i in range(canvas_height):
            for j in range(canvas_width):
                next_grid[i, j, :] = (intensity(random_f(i, j, u1)), intensity(random_f(i, j, u2)), intensity(random_f(i, j, u3)))
                # greyscale version
